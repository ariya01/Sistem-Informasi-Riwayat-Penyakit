...
<!--                 <div class="row purchace-popup">
        ...
        <div style="margin-right: 20%;">
          <a href="{{url('ubah1/'.$a->id_riwayat.'/'.$nama->id)}}"> 
            <i style="color: green;" class="fa fa-pencil-square-o" aria-hidden="true"></i>
          </a>
        </div>
        <div> 
          <a href="{{url('deletekok/'.$a->id_riwayat.'/'.$nama->id)}}" onclick="myFunction(event);"><i style="color: red;" class="fa fa-times" aria-hidden="true"></i></a>
        </div>
      </div>
      </td>
              <td>{{$a->S}}</td>
...
<!--               <td>{{$a->created_at}}</td>
 -->            </tr>
        ...
        <a href="{{url('ubah1/'.$angka.'/'.$nama->id)}}">
    <i class="fa fa-plus" style="margin-left: 2%; margin-top: 1%" aria-hidden="true"></i>
    <span style="font-size: 80%;">Tambah Pemeriksaan</span>
    </a>
      </div>
    </div>
  </div>
...
// <script type="text/javascript">
//   $(document).ready( function () {
//    $('#myTable').DataTable({ order: [[1, 'asc']],bPaginate: $('#myTable tbody tr').length>10,"columnDefs": [
//     { "width": "10%", "targets": 0 }
//   ]} );
//   } );
</script>
