...
<div class="col-12">
  <div
  class="card">
     <a href="{{url('lihat2/'.$id_user)}}">
      <i class="fa fa-arrow-left" style="margin-left: 2%; margin-top: 1%" aria-hidden="true"></i>
      <span style="font-size: 80%;"> Kembali ke Riwayat Pemeriksaan </span>
    </a>
  <div class="card-body">
    <h4 class="card-title">Riwayat Pemeriksaan</h4>
    <form class="forms-sample" method="post" action="{{url('kirim4')}} ">
     <div class="form-group">
      ...
      <div class="form-group">
        <label for="exampleInputPassword1">Objek</label>
        <div class="input-group">
          <div class="input-group-prepend">
          </div>
      ...
      <div class="form-group">
        <label for="exampleInputPassword1">Assesmen</label>
        <div class="input-group">
          <div class="input-group-prepend">
          </div>
...
@if ($data!=null)
<input type="hidden" name="id" id="id" value="{{$data->id_riwayat}}">
@endif
@endsection
@section('js')
<script type="text/javascript">
            ...
            if(bla)
            {
              $.ajax({    
                ...
                success:function(data){
                  ...
                  $("#subyek").val(JSON.parse(data).S);
                  $("#objek").val(JSON.parse(data).O);
                  $("#asssesmen").val(JSON.parse(data).A);
                  $("#plan").val(JSON.parse(data).P);
                  // $("#tanggal").val(JSON.parse(data).created_at);
                  $("#id_riwayat").val(JSON.parse(data).id_riwayat);
                },
                error:function(data){
